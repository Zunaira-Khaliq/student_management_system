...
async function studentBalance() {
  ...
  const findStudent = students.find((s:any) => s.id === studentId);
  if (findStudent) {
    console.log(
...
async function topUpBalance() {
  ...
  const studentById = students.find(
    (s:any) => s.id === parseInt(studentIdForTopUp.studentId)
  );
  if (studentById) {
...
async function payFee() {
  ...
  if (!confirmPayment) {
    ...
    const studentDetails = students.find(
      (s:any) => s.id === parseInt(PayTuitionFee.studentId)
    );
    if (studentDetails) {
...
async function showStatus() {
  ...
  const matchedStudent = students.find((s:any) => s.id === studentId);
  if (matchedStudent) {
