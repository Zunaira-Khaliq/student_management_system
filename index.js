...
function displayWelcome() {
    console.log("Welcome to Zydo Student Management System!");
}
...
async function studentBalance() {
    ...
    if (findStudent) {
        console.log(`Account balance for student with ID ${studentId}: $${findStudent.balance.toFixed(0)}, Please top up your account.`);
    }
    else {
        console.log("Student not found.");
    }
...
async function topUpBalance() {
    ...
    const studentById = students.find((s) => s.id === parseInt(studentIdForTopUp.studentId));
    if (studentById) {
        studentById.balance += topUpAmount;
        console.log(`Balance topped up! New balance: $${studentById.balance.toFixed(0)}`);
    }
    else {
...
async function payFee() {
    ...
    if (!confirmPayment) {
        ...
        const studentDetails = students.find((s) => s.id === parseInt(PayTuitionFee.studentId));
        if (studentDetails) {
